...
type info =
  ...
  | InfoTypedef of string * typ (*ajouté pour la gestion des types nommés*)
  | InfoChamps of string * typ * typ * int (*ajouté pour la gestion des enregistrements*)
...
let string_of_info info =
  ...
  | InfoFun (n,t,tp) -> "Fonction "^n^" : "^(List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt)^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)
  | InfoTypedef (n,t) -> "Type "^n^" : "^(string_of_type t)
  | InfoChamps(n,t,type_enregistrement,dep) -> "Champs " ^ n ^ " : " ^ (string_of_type t) ^ " de " ^ (string_of_type type_enregistrement) ^ " à un déplacement de " ^ (string_of_int dep)
...
(* Modifie le type si c'est une InfoVar, ne fait rien sinon *)
  let modifier_type_info t i =
    ...
    |InfoTypedef (n,_) -> i:= InfoTypedef (n,t)
...
(* Modifie l'emplacement (dépl) si c'est une InfoChamps, ne fait rien sinon *)
let modifier_adresse_info_champs d i =
  match !i with
    |InfoChamps(n,t,te,_) -> i:=InfoChamps(n,t,te,d)
    |_ -> failwith "Appel modifier_adresse_info pas sur un Infochamps"
